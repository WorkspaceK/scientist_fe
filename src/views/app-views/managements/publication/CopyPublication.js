...
const CopyPublication = () => {
    ...
    const fetchData = async () => {
        const response = await publicationAPI.show(res.id);
        setDataPublication(response);
    ...
    const update = async (data) => {
        try {
            const res = await publicationAPI.update(res.id, data);
    ...
    const rules = {
        identification: [
            ...
            { max: 50, message: 'CCCD/CMND không được vượt quá 50 ký tự' },
            {
                validator: async (_, value) => {
                    if (value === dataPublication.identification) {
                        return Promise.resolve();
                    } else {
                        const exists = await throttledSearchByIdentifi(value);
                        if (exists === true && value !== dataPublication.identification) {
        ...
        email: [
            ...
            {
                validator: async (_, value) => {
                    if (value === dataPublication.email) {
                        return Promise.resolve();
                    } else {
                        const exists = await throttledSearchByIdentifi(value);
                        if (exists === true && value !== dataPublication.email) {
